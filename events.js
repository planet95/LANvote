var config = require('./config')
  ...
  , findByUsername = exports.findByUsername = function(username, callback) {
      ...
      findBy('byUsername', {key: username}, function(err, event) {
          if (err) {
            callback(err, null);
          }
          else {
  ...
  , voteCounts = exports.voteCounts = function(event, callback) {
      db.view('games', 'all', {startkey: [event._id], endkey: [event._id, {}, {}], group_level: 2}, function(err, body) {
        if (err) {
          callback(err);
        }
        else {
