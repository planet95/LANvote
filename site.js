 $(document).ready(function(){
   ...
   $.each(obj, function(name, item) {
          items.push('<div id="li_'+name+'"><a>' + name + ' Votes: '+ item.votes+'</a><button style="background-image: url('+item.icon+')"  onclick=vote(' + item.votes + ',"'+name+'") class="punch"></button></div>');
...
function vote(id, name) {
   $('#li_'+ name + ' a').html(name + ' Votes:' + (parseInt(id) + 1));

    $('#li_'+ name + ' a').click(function(e){e.preventDefault();});
}